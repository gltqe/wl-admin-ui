<template>
  <div class="layout-content">
    <el-row class="search">
      ...
      >
        ...
        <el-form-item label="键" prop="code">
          <el-input v-model="queryForm.code" clearable placeholder="请输入参数键"></el-input>
        </el-form-item>
        <!--        <el-form-item label="值" prop="value">-->
        <!--          <el-input v-model="queryForm.value" clearable placeholder="请输入参数值"></el-input>-->
        <!--        </el-form-item>-->
        <el-form-item label="类型" prop="type">
          <wl-select v-model="queryForm.type" @change="handleTableData" placeholder="请选择类型"
                     dict-code="config_type" clearable></wl-select>
        </el-form-item>
    ...
    <el-row class="operate">
      <el-button plain size="small" icon="Plus" type="primary" v-permission="'config:list:add'" @click="handleAdd">新增</el-button>
      <el-button plain size="small" icon="Edit" type="warning" v-permission="'config:list:update'" @click="handleUpdate" :disabled="!isRadio">修改</el-button>
      <el-button plain size="small" icon="Delete" type="danger" v-permission="'config:list:remove'" @click="handleDelete" :disabled="!isSelect">删除</el-button>
      <el-button plain size="small" icon="Upload" type="info" v-permission="'config:list:import'">导入</el-button>
      <el-button plain size="small" icon="Download" type="success" v-permission="'config:list:export'">导出</el-button>
      <el-button plain size="small" icon="Refresh" type="danger" v-permission="'config:list:refresh'" @click="handleRefresh">刷新缓存</el-button>
    </el-row>
    ...
    >
      ...
      <el-table-column label="键" align="center" key="code" prop="code" sortable="custom"/>
      <el-table-column label="值" align="center" key="value" prop="value" sortable="custom"/>
      <el-table-column label="类型" align="center" key="type" prop="type" sortable="custom">
        <template #default="scope">
          <wl-dict :data-map="config_type" :value="scope.row.type"></wl-dict>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" v-permission="'config:list:update'" @click="handleUpdate(scope.row)">修改</el-button>
          <el-button link type="primary" icon="Delete" v-permission="'config:list:remove'" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    >
      ...
      >
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="键" prop="code">
              <el-input v-model="form.code" placeholder="请输入参数键"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="值" prop="value">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="类型" prop="type">
              <wl-select v-model="form.type" dict-code="config_type" placeholder="请选择类型"></wl-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="状态" prop="status">
...
import {page, getOne, remove, add, update,refresh} from "@/api/system/config.js";
...
const {sys_status,config_type} = getDictMap('sys_status','config_type')
...
const handleDelete = (row) => {
  let msg = '删除后无法找回,是否确定要删除选中的数据?'
  if (row.type === '0') {
    msg = '所选数据中包含系统参数,删除后可能导致系统异常,是否确定要删除选中的数据?'
  }
  if (selectData.value && selectData.value.length > 0) {
    for (let i = 0; i < selectData.value.length; i++) {
      if (selectData.value[i].type === '0') {
        msg = '所选数据中包含系统参数,删除后可能导致系统异常,是否确定要删除选中的数据?'
        break;
      }
    }
