...
<!--    </el-row>-->
    ...
    >
      ...
      <el-table-column label="部门名" align="center" key="deptName" prop="deptName" sortable="custom"/>
      <el-table-column label="登录时间" align="center" key="loginTime" prop="loginTime" sortable="custom"/>
      <el-table-column label="ip" align="center" key="ip" prop="ip" sortable="custom"/>
      <el-table-column label="浏览器" align="center" key="browser" prop="browser" sortable="custom"/>
      <el-table-column label="系统" align="center" key="os" prop="os" sortable="custom"/>
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handleExit(scope.row)">强制退出</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="状态">
          <template #default>
            <wl-dict :data-map="user_status" :value="form.status"></wl-dict>
          </template>
        </el-descriptions-item>
...
import {getOnlineUserPage, getOne, exit} from "@/api/monitor/onlineUser.js";
...
const handleTableData = () => {
  ...
  getOnlineUserPage(queryForm.value).then(res => {
...
const handleExit = (row) => {
  ElMessageBox.confirm('确定要强制退出['+row.username+']用户?', '警告', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(() => {
    // const ids = getCheckboxParam(row, selectData.value, 'id')
    exit({username:row.username}).then(res => {
      handleResult(res)
      handleTableData()
    })
  }).catch(() => {
